...
// W2 Warehouse Category Mapping
const W2_CATEGORIES = {
    'W2-001': 'Bricks & Masonry',
    'W2-002': 'Solid Cement Blocks',
    'W2-004': 'Clay Bricks',
    'W2-008': 'Masonry Blocks'
};
...
const getCategoryId = (materialName: string): string => {
    ...
    return 'W2-001'; // Default to general category
};

// Helper function to get category name from ID
const getCategoryName = (categoryId: string): string => {
    return W2_CATEGORIES[categoryId as keyof typeof W2_CATEGORIES] || 'Bricks & Masonry';
};

interface SubOrderItem {
    materialId: string;
    materialName: string;
    categoryId?: string; // Warehouse category ID (e.g., W2-002, W2-004)
    quantity: number;
    unitPrice: number;
    totalPrice: number;
    _id: string;
...
const W2SubOrdersPage: React.FC = () => {
    const { user } = useAuth();
    ...
    useEffect(() => {
        console.log('W2 Sub-Orders - Current user:', user);
        console.log('W2 Sub-Orders - Warehouse code:', user?.warehouseCode);
    ...
    useEffect(() => {
        ...
        if (user && !loading && !error) {
            intervalId = setInterval(() => {
                console.log('Auto-refreshing W2 sub-orders...');
                fetchW2SubOrders();
            }, 30000);
        }
    ...
    const fetchW2SubOrders = async () => {
        try {
            ...
            console.log('Fetching W2 warehouse SubOrders from SubOrders collection:', url);
            ...
            console.log('W2 Warehouse SubOrders Response data:', data);

            if (data.success) {
                if (data.warehouseCode === 'W2' || user?.warehouseCode === 'W2' || user?.email?.includes('house2')) {
                    setSubOrders(data.subOrders || []);
                    console.log(`Found ${data.subOrders?.length || 0} warehouse items for W2 warehouse`);
                } else if (data.warehouseCode && data.warehouseCode !== 'W2') {
                    setError(`Access denied. This page is for W2 (Blocks & Masonry) warehouse items, but you are assigned to ${data.warehouseCode} warehouse.`);
                } else {
                    setSubOrders(data.subOrders || []);
                }
            } else {
                setError(data.error || 'Failed to fetch W2 warehouse items');
            }
        } catch (err) {
            console.error('Error fetching W2 warehouse items:', err);
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            <h1 className="text-3xl font-bold text-gray-900 dark:text-white flex items-center">
                                ...
                                W2 - Blocks & Masonry Sub-Orders
                            </h1>
                            <p className="text-gray-600 dark:text-gray-300 mt-2">
                                Manage all sub-orders for blocks, bricks, and masonry materials
                            </p>
                        </div>
                        <div className="text-right">
                            <p className="text-sm text-gray-600 dark:text-gray-300">Warehouse: <span className="font-bold text-orange-600">W2</span></p>
                            <p className="text-sm text-gray-600 dark:text-gray-300">User: {user?.email}</p>
                            <button
                                onClick={fetchW2SubOrders}
                                disabled={loading}
                                className="mt-2 px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 text-sm"
                            >
                                {loading ? (
                                    <>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm">
                    <div className="p-6 border-b border-gray-200 dark:border-gray-700">
                        <h2 className="text-xl font-semibold text-gray-900 dark:text-white flex items-center">
                            ...
                            Blocks & Masonry Sub-Orders ({subOrders.length})
                        </h2>
                        <p className="text-gray-600 dark:text-gray-300 mt-1">
                            All sub-orders assigned to W2 warehouse for blocks, bricks, and masonry materials
                        </p>
                    </div>
                    ...
                    {subOrders.length === 0 ? (
                        <div className="p-12 text-center">
                            ...
                            <p className="text-gray-600 dark:text-gray-300">
                                There are currently no sub-orders for W2 warehouse.
                            </p>
                        </div>
                    ) : (
                                    ...
                                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
                                        <div>
                                            ...
                                            <div className="flex items-center gap-4 text-sm text-gray-600 dark:text-gray-300">
                                                ...
                                                <span className="bg-orange-100 dark:bg-orange-800 px-2 py-1 rounded text-xs font-mono">
                                                    Warehouse: W2
                                                </span>
                                                {subOrder._id && (
                                    ...
                                    <div className="mb-4">
                                        ...
                                        <div className="space-y-2">
                                            {subOrder.items.map((item) => {
                                                ...
                                                return (
                                                    <div key={item._id} className="p-3 bg-orange-50 dark:bg-orange-900 rounded-lg border-l-4 border-orange-500">
                                                        <div className="flex justify-between items-start mb-2">
                                                            <div className="flex-1">
                                                                ...
                                                                <div className="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400">
                                                                    <span className="bg-orange-100 dark:bg-orange-800 px-2 py-1 rounded font-mono">
                                                                        {categoryId}
                                                                    </span>
                                                                    <span>•</span>
...
export default W2SubOrdersPage;
