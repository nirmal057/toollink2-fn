      ...
      if (result.success) {
        ...
        if (result.errorType === 'ACCOUNT_PENDING_APPROVAL') {
          addToast({
            ...
            message: result.error || 'Your account is waiting for approval by an administrator. You will receive an email notification once your account is approved and you can login.',
            duration: 8000
          });
          setError(result.error || 'Your account is pending approval. Please wait for administrator approval before logging in.');
        } else if (result.errorType === 'INVALID_CREDENTIALS') {
          // Show popup-style toast for wrong credentials
          addToast({
            type: 'error',
            title: '❌ Login Failed',
