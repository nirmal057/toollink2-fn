...
import { Calendar, Clock, Package, Truck, CheckCircle, AlertCircle, XCircle, User, Wrench } from 'lucide-react';
import { useAuth } from '../hooks/useAuth';
import { API_CONFIG } from '../config/api';

// WM Warehouse Category Mapping
const WM_CATEGORIES = {
    'WM-001': 'Tools & Equipment',
    'WM-004': 'Power Drills',
    'WM-011': 'Angle Grinders',
    'WM-012': 'Cement',
    'WM-018': 'Hardware & Fasteners'
};
...
const getCategoryId = (materialName: string): string => {
    ...
    if (name.includes('hardware') || name.includes('fastener') || name.includes('bolt') || name.includes('screw')) return 'WM-018';

    return 'WM-001'; // Default to general category
};

// Helper function to get category name from ID
const getCategoryName = (categoryId: string): string => {
    return WM_CATEGORIES[categoryId as keyof typeof WM_CATEGORIES] || 'Tools & Equipment';
};

interface SubOrderItem {
    materialId: string;
    materialName: string;
    categoryId?: string; // Warehouse category ID (e.g., WM-004, WM-011)
    quantity: number;
    unitPrice: number;
    totalPrice: number;
    _id: string;
...
const WMSubOrdersPage: React.FC = () => {
    ...
    useEffect(() => {
        console.log('WM Sub-Orders - Current user:', user);
        console.log('WM Sub-Orders - Warehouse code:', user?.warehouseCode);
    ...
    const fetchWMSubOrders = async () => {
        try {
            ...
            console.log('Fetching WM warehouse SubOrders from SubOrders collection:', url);
            ...
            console.log('WM Warehouse SubOrders Response data:', data);

            if (data.success) {
                if (data.warehouseCode === 'WM' || user?.warehouseCode === 'WM' || user?.email?.includes('warehouse-main')) {
                    setSubOrders(data.subOrders || []);
                    console.log(`Found ${data.subOrders?.length || 0} warehouse items for WM warehouse`);
                } else if (data.warehouseCode && data.warehouseCode !== 'WM') {
                    setError(`Access denied. This page is for WM (Tools & Equipment) warehouse items, but you are assigned to ${data.warehouseCode} warehouse.`);
                } else {
                    setSubOrders(data.subOrders || []);
                }
            } else {
                setError(data.error || 'Failed to fetch WM warehouse items');
            }
        } catch (err) {
            console.error('Error fetching WM warehouse items:', err);
    ...
    const getStatusIcon = (status: string) => {
        switch (status.toLowerCase()) {
            ...
            case 'prepared': return <Wrench className="w-5 h-5 text-yellow-500" />;
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            <h1 className="text-3xl font-bold text-gray-900 dark:text-white flex items-center">
                                ...
                                WM - Tools & Equipment Sub-Orders
                            </h1>
                            <p className="text-gray-600 dark:text-gray-300 mt-2">
                                Manage all sub-orders for tools, equipment, and machinery
                            </p>
                        </div>
                        <div className="text-right">
                            <p className="text-sm text-gray-600 dark:text-gray-300">Warehouse: <span className="font-bold text-yellow-600">WM</span></p>
                            <p className="text-sm text-gray-600 dark:text-gray-300">User: {user?.email}</p>
                            <button
                                onClick={fetchWMSubOrders}
                                disabled={loading}
                                className="mt-2 px-4 py-2 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 text-sm"
                            >
                                {loading ? (
                                    <>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm">
                    <div className="p-6 border-b border-gray-200 dark:border-gray-700">
                        <h2 className="text-xl font-semibold text-gray-900 dark:text-white flex items-center">
                            ...
                            Tools & Equipment Sub-Orders ({subOrders.length})
                        </h2>
                        <p className="text-gray-600 dark:text-gray-300 mt-1">
                            All sub-orders assigned to WM warehouse for tools, equipment, and machinery
                        </p>
                    </div>
                    ...
                    {subOrders.length === 0 ? (
                        <div className="p-12 text-center">
                            ...
                            <p className="text-gray-600 dark:text-gray-300">
                                There are currently no sub-orders for WM warehouse.
                            </p>
                        </div>
                    ) : (
                                    ...
                                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
                                        <div>
                                            ...
                                            <div className="flex items-center gap-4 text-sm text-gray-600 dark:text-gray-300">
                                                ...
                                                <span className="bg-yellow-100 dark:bg-yellow-800 px-2 py-1 rounded text-xs font-mono">
                                                    Warehouse: WM
                                                </span>
                                                {subOrder._id && (
                                    ...
                                    <div className="mb-4">
                                        ...
                                        <div className="space-y-2">
                                            {subOrder.items.map((item) => {
                                                ...
                                                return (
                                                    <div key={item._id} className="p-3 bg-yellow-50 dark:bg-yellow-900 rounded-lg border-l-4 border-yellow-500">
                                                        <div className="flex justify-between items-start mb-2">
                                                            <div className="flex-1">
                                                                ...
                                                                <div className="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-400">
                                                                    <span className="bg-yellow-100 dark:bg-yellow-800 px-2 py-1 rounded font-mono">
                                                                        {categoryId}
                                                                    </span>
                                                                    <span>•</span>
...
export default WMSubOrdersPage;
