...
import { Calendar, Clock, Package, Truck, CheckCircle, AlertCircle, XCircle, User, Eye, MapPin } from 'lucide-react';
...
const W1WarehousePage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (user && user.warehouseCode === 'W1') {
            fetchW1SubOrders();
        } else if (user && user.warehouseCode !== 'W1') {
            setError('Access denied. This page is only for W1 (Sand & Aggregates) warehouse staff.');
        }
    }, [user, filter]);
    ...
    const fetchW1SubOrders = async () => {
        try {
            ...
            const url = `${API_CONFIG.BASE_URL}/api/orders/sub-orders/warehouse/W1?${params}`;
            console.log('Fetching W1 sub-orders from:', url);
            ...
            console.log('W1 Response data:', data);

            if (data.success) {
                setSubOrders(data.subOrders || []);
                console.log(`Found ${data.subOrders?.length || 0} sub-orders for W1 warehouse`);
            } else {
                setError(data.error || 'Failed to fetch W1 sub-orders');
            }
        } catch (err) {
            console.error('Error fetching W1 sub-orders:', err);
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-6">
            <div className="max-w-7xl mx-auto">
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            <h1 className="text-3xl font-bold text-gray-900 dark:text-white flex items-center">
                                ...
                                W1 - Sand & Aggregates Warehouse
                            </h1>
                            <p className="text-gray-600 dark:text-gray-300 mt-2">
                                Manage your sand and aggregates deliveries and sub-orders
                            </p>
                        </div>
                ...
                {!error && user?.warehouseCode === 'W1' && (
                    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm mb-6">
                        <div className="p-6 border-b border-gray-200 dark:border-gray-700">
                            <h2 className="text-xl font-semibold text-gray-900 dark:text-white flex items-center">
                                <Package className="w-6 h-6 text-blue-600 mr-2" />
                                W1 Warehouse - Sand & Aggregate Category Distribution
                            </h2>
                            <p className="text-gray-600 dark:text-gray-300 mt-1">
                                Your warehouse's sand and aggregate categories
                            </p>
                        </div>
                        <div className="p-6">
