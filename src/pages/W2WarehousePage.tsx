...
const W2WarehousePage: React.FC = () => {
    ...
    const fetchW2SubOrders = async () => {
        try {
            ...
            if (data.success) {
                // Verify this is W2 warehouse data or accept any warehouse for the logged-in user
                if (data.warehouseCode === 'W2' || user?.warehouseCode === 'W2' || user?.email?.includes('house2')) {
                    ...
                    setError(`Access denied. This page is for W2 (Blocks & Masonry) warehouse, but you are assigned to ${data.warehouseCode} warehouse.`);
                } else {
                    setSubOrders(data.subOrders || []);
                }
            } else {
