...
const W2WarehousePage: React.FC = () => {
    const { user } = useAuth();
    ...
    useEffect(() => {
        console.log('W2 Warehouse - Current user:', user);
        console.log('W2 Warehouse - Warehouse code:', user?.warehouseCode);

        if (user && user.warehouseCode === 'W2') {
            fetchW2SubOrders();
        } else if (user && user.warehouseCode !== 'W2') {
            setError('Access denied. This page is only for W2 (Blocks & Masonry) warehouse staff.');
        }
    }, [user, filter]);
    ...
    const fetchW2SubOrders = async () => {
        try {
            ...
            const url = `${API_CONFIG.BASE_URL}/api/orders/sub-orders/warehouse/W2?${params}`;
            console.log('Fetching W2 sub-orders from:', url);
            ...
            console.log('W2 Response data:', data);

            if (data.success) {
                setSubOrders(data.subOrders || []);
                console.log(`Found ${data.subOrders?.length || 0} sub-orders for W2 warehouse`);
            } else {
                setError(data.error || 'Failed to fetch W2 sub-orders');
            }
        } catch (err) {
            console.error('Error fetching W2 sub-orders:', err);
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-6">
            <div className="max-w-7xl mx-auto">
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            <h1 className="text-3xl font-bold text-gray-900 dark:text-white flex items-center">
                                ...
                                W2 - Blocks & Masonry Warehouse
                            </h1>
                            <p className="text-gray-600 dark:text-gray-300 mt-2">
                                Manage your blocks, bricks, and masonry deliveries and sub-orders
                            </p>
                        </div>
                        <div className="text-right">
                            <p className="text-sm text-gray-600 dark:text-gray-300">Warehouse Code: <span className="font-bold text-orange-600">W2</span></p>
                ...
                {!error && user?.warehouseCode === 'W2' && (
                    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm mb-6">
                        <div className="p-6 border-b border-gray-200 dark:border-gray-700">
                            <h2 className="text-xl font-semibold text-gray-900 dark:text-white flex items-center">
                                <Package className="w-6 h-6 text-orange-600 mr-2" />
                                W2 Warehouse - Bricks & Masonry Category Distribution
                            </h2>
                            <p className="text-gray-600 dark:text-gray-300 mt-1">
                                Your warehouse's bricks, masonry and stone categories
                            </p>
                        </div>
                        <div className="p-6">
                                        ...
                                        <div className="mb-4">
                                            ...
                                            <div className="space-y-2">
                                                {subOrder.items.map((item) => (
                                                    <div key={item._id} className="flex justify-between items-center p-2 bg-orange-50 dark:bg-orange-900 rounded">
...
export default W2WarehousePage;
